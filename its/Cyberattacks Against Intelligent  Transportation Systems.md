...
* Transit signal priority (TSP) — 大眾運輸號誌優先系統Is a set of operational improvements that reduce wait time at traffic signals for transit vehicles by holding green lights longer or shortening red lights. TSP may be implemented at individual intersections, across corridors, or entire street systems.
* 是一組操作改進，通過延長綠燈或縮短紅燈來減少交通車輛在交通號誌處的等待時間。TSP 可以在單一交叉路口、跨走廊或整個街道系統實施。
